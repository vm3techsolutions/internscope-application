...
//        });
        ...
        holder.tvViewResume.setOnClickListener(v -> {
            ...
            String resumeKey = candidate.getResumeUrl();
            ...
            String token = sm.getActiveToken();

            ApiService api = ApiClient.getClient(context).create(ApiService.class);

            api.getResumeViewUrl("Bearer " + token, resumeKey)
                    .enqueue(new Callback<UploadResponse>() {
                        @Override
                        public void onResponse(Call<UploadResponse> call, Response<UploadResponse> response) {
                            if (response.isSuccessful() && response.body() != null) {
                                String signedUrl = response.body().getUrl(); // from your backend
                                ...
                                Toast.makeText(context, "Failed to fetch resume", Toast.LENGTH_SHORT).show();
                            }
                        }
